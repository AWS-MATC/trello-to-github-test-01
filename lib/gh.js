...
GH.mustInit = async function () {
  ...
  let fields = await GH.projects.getTableFields({ id: project.id });
...
GH.projects.getByName = async function ({ name }) {
  ...
  let board = projectBoards.data.organization.projects.nodes.find(function (node) {
...
GH.projects.getTableFields = async function ({name = process.env.GITHUB_PROJECT_NAME, projectNodeId }) {
  if (!projectNodeId) {
    projectNodeId = await GH.projects.getNodeIdByName({ name });
  }
